...
package org.apache.spark.mllibnew.linalg
...
sealed trait Matrix extends Serializable {
  ...
  private[mllibnew] def toBreeze: BM[Double]

  /** Gets the (i, j)-th element. */
  private[mllibnew] def apply(i: Int, j: Int): Double

  /** Return the index for the (i, j)-th element in the backing array. */
  private[mllibnew] def index(i: Int, j: Int): Int

  /** Update element at (i, j) */
  private[mllibnew] def update(i: Int, j: Int, v: Double): Unit
...
class DenseMatrix(val numRows: Int, val numCols: Int, val values: Array[Double]) extends Matrix {
  ...
  private[mllibnew] def toBreeze: BM[Double] = new BDM[Double](numRows, numCols, values)

  private[mllibnew] def apply(i: Int): Double = values(i)

  private[mllibnew] def apply(i: Int, j: Int): Double = values(index(i, j))

  private[mllibnew] def index(i: Int, j: Int): Int = i + numRows * j
...
/**
 * Factory methods for [[org.apache.spark.mllibnew.linalg.Matrix]].
 */
object Matrices {
   ...
   */
  private[mllibnew] def fromBreeze(breeze: BM[Double]): Matrix = {
    breeze match {
      case dm: BDM[Double] =>
