...
import org.apache.spark.mllibnew.stat.{MultivariateOnlineSummarizer, MultivariateStatisticalSummary}
...
class RowMatrix(
   ...
   */
  private[mllibnew] def multiplyGramianMatrixBy(v: BDV[Double]): BDV[Double] = {
    val n = numCols().toInt
   ...
   * @return SingularValueDecomposition(U, s, V). U = null if computeU = false.
   */
  private[mllibnew] def computeSVD(
      k: Int,
      computeU: Boolean,
      rCond: Double,
      maxIter: Int,
   ...
   * @return a [[org.apache.spark.mllibnew.linalg.distributed.RowMatrix]] representing the product,
