...
package org.apache.spark.mllib2.linalg
...
sealed trait Matrix extends Serializable {
  ...
  private[mllib2] def toBreeze: BM[Double]

  /** Gets the (i, j)-th element. */
  private[mllib2] def apply(i: Int, j: Int): Double

  /** Return the index for the (i, j)-th element in the backing array. */
  private[mllib2] def index(i: Int, j: Int): Int

  /** Update element at (i, j) */
  private[mllib2] def update(i: Int, j: Int, v: Double): Unit
...
class DenseMatrix(val numRows: Int, val numCols: Int, val values: Array[Double]) extends Matrix {
  ...
  private[mllib2] def toBreeze: BM[Double] = new BDM[Double](numRows, numCols, values)

  private[mllib2] def apply(i: Int): Double = values(i)

  private[mllib2] def apply(i: Int, j: Int): Double = values(index(i, j))

  private[mllib2] def index(i: Int, j: Int): Int = i + numRows * j
...
/**
 * Factory methods for [[org.apache.spark.mllib2.linalg.Matrix]].
 */
object Matrices {
   ...
   */
  private[mllib2] def fromBreeze(breeze: BM[Double]): Matrix = {
    breeze match {
      case dm: BDM[Double] =>
